...
function buildCharSvg(o) {
  ...
  return buildCharSvgInner(o, shirt, pack, accessory, cap);
}

// Variant used on the onboarding menu where the character is HANGING from a
// rock by their hands. Same outfit data → same colors and cap; the pose draws
// articulated hands gripping at the top, arms reaching up-out from shoulders,
// head looking up, body and legs dangling below.
function buildClimberSvg(o) {
  const skin = '#d8a878', skinDark = '#7a4a2a';

  let cap = '';
  if (o.capStyle === 'helmet') {
    cap = `<circle cx="0" cy="-30" r="12" fill="${o.cap}" stroke="${o.capDark}" stroke-width="1.4"/>
           <ellipse cx="-1" cy="-30" rx="8" ry="5" fill="#3a4a6a" opacity="0.6"/>`;
  } else if (o.capStyle === 'toque') {
    cap = `<ellipse cx="0" cy="-46" rx="8" ry="8" fill="${o.cap}" stroke="${o.capDark}" stroke-width="0.9"/>
           <ellipse cx="0" cy="-38" rx="9" ry="3" fill="${o.cap}" stroke="${o.capDark}" stroke-width="0.9"/>
           <rect x="-9" y="-36" width="18" height="3" fill="${o.capCuff}"/>`;
  } else if (o.capStyle === 'hardhat') {
    cap = `<path d="M-12 -32 Q-12 -40 0 -40 Q12 -40 12 -32 Z" fill="${o.cap}" stroke="${o.capDark}" stroke-width="1.2"/>
           <rect x="-13" y="-33" width="26" height="2" fill="${o.cap}" stroke="${o.capDark}" stroke-width="0.7"/>
           <rect x="-2" y="-40" width="4" height="6" fill="${o.capDark}"/>`;
  } else if (o.capStyle === 'fedora') {
    cap = `<ellipse cx="0" cy="-32" rx="13" ry="2.5" fill="${o.cap}" stroke="${o.capDark}" stroke-width="1"/>
           <rect x="-8" y="-40" width="16" height="8" rx="2" fill="${o.cap}" stroke="${o.capDark}" stroke-width="1"/>
           <rect x="-8" y="-34" width="16" height="2" fill="${o.capCuff}"/>`;
  } else if (o.capStyle === 'headphones') {
    cap = `<path d="M-11 -32 Q-11 -40 0 -40 Q11 -40 11 -32" fill="none" stroke="${o.cap}" stroke-width="2"/>
           <ellipse cx="-11" cy="-30" rx="3" ry="4" fill="${o.cap}" stroke="${o.capDark}" stroke-width="0.8"/>
           <ellipse cx="11" cy="-30" rx="3" ry="4" fill="${o.cap}" stroke="${o.capDark}" stroke-width="0.8"/>`;
  } else if (o.capStyle === 'goggles') {
    cap = `<ellipse cx="-4" cy="-28" rx="3" ry="2.5" fill="#aaccff" stroke="${o.capDark}" stroke-width="0.7"/>
           <ellipse cx="4" cy="-28" rx="3" ry="2.5" fill="#aaccff" stroke="${o.capDark}" stroke-width="0.7"/>
           <line x1="-1" y1="-28" x2="1" y2="-28" stroke="${o.capDark}" stroke-width="0.6"/>`;
  } else if (o.capStyle === 'beanie') {
    cap = `<path d="M-9 -30 Q-10 -40 0 -40 Q10 -40 9 -30 Z" fill="${o.cap}" stroke="${o.capDark}" stroke-width="1"/>
           <rect x="-9" y="-32" width="18" height="3" fill="${o.capCuff}" stroke="${o.capDark}" stroke-width="0.7"/>`;
  } else {
    cap = `<path d="M-8 -34 Q-9 -38 0 -38 Q9 -38 8 -34 L8 -30 L-8 -30 Z" fill="${o.hair}"/>`;
  }

  const belt = (o.shirtStyle === 'labcoat' || o.shirtStyle === 'spacesuit' || o.shirtStyle === 'chef') ? '' :
    `<rect x="-13" y="6" width="26" height="3.5" fill="#3a1f0a" stroke="#1a0c04" stroke-width="0.4"/>
     <rect x="-2" y="6" width="4" height="3.5" fill="#c8a060"/>`;

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="-36 -54 72 108">
    <!-- BOOTS (legs slightly bent against the rock) -->
    <ellipse cx="-11" cy="44" rx="6.5" ry="3.2" fill="${o.bootDark}"/>
    <ellipse cx="11" cy="44" rx="6.5" ry="3.2" fill="${o.bootDark}"/>
    <ellipse cx="-11" cy="42" rx="6" ry="2.8" fill="${o.boot}" stroke="${o.bootDark}" stroke-width="0.5"/>
    <ellipse cx="11" cy="42" rx="6" ry="2.8" fill="${o.boot}" stroke="${o.bootDark}" stroke-width="0.5"/>
    <!-- PANTS / legs -->
    <path d="M-12 9 L-12 40 L-4 40 L-3 9 Z" fill="${o.pants}" stroke="${o.pantsDark}" stroke-width="0.7"/>
    <path d="M12 9 L12 40 L4 40 L3 9 Z"     fill="${o.pants}" stroke="${o.pantsDark}" stroke-width="0.7"/>
    <rect x="-12" y="37" width="8" height="3" fill="${o.cuff}" stroke="${o.pantsDark}" stroke-width="0.4"/>
    <rect x="4"  y="37" width="8" height="3" fill="${o.cuff}" stroke="${o.pantsDark}" stroke-width="0.4"/>
    ${belt}
    <!-- TORSO (shirt) -->
    <path d="M-13 -18 Q-14 -8 -13 6 L13 6 Q14 -8 13 -18 Q6 -20 0 -20 Q-6 -20 -13 -18 Z"
          fill="${o.shirt}" stroke="${o.shirtDark}" stroke-width="1.2"/>
    <!-- ARMS reaching UP-OUT to grip rock peak -->
    <path d="M-13 -18 Q-19 -32 -22 -44" stroke="${o.shirt}" stroke-width="7" fill="none" stroke-linecap="round"/>
    <path d="M-13 -18 Q-19 -32 -22 -44" stroke="${o.shirtDark}" stroke-width="0.7" fill="none" stroke-linecap="round" opacity="0.4"/>
    <path d="M13 -18 Q19 -32 22 -44"   stroke="${o.shirt}" stroke-width="7" fill="none" stroke-linecap="round"/>
    <path d="M13 -18 Q19 -32 22 -44"   stroke="${o.shirtDark}" stroke-width="0.7" fill="none" stroke-linecap="round" opacity="0.4"/>
    <!-- HANDS (fist clutching, with knuckle bumps) -->
    <ellipse cx="-22" cy="-44" rx="5.2" ry="4" fill="${skin}" stroke="${skinDark}" stroke-width="0.7"/>
    <path d="M-26 -45 Q-25 -47 -24 -45 Q-23 -47 -22 -45 Q-21 -47 -20 -45 Q-19 -47 -18 -45"
          stroke="${skinDark}" stroke-width="0.5" fill="none"/>
    <ellipse cx="22" cy="-44" rx="5.2" ry="4" fill="${skin}" stroke="${skinDark}" stroke-width="0.7"/>
    <path d="M18 -45 Q19 -47 20 -45 Q21 -47 22 -45 Q23 -47 24 -45 Q25 -47 26 -45"
          stroke="${skinDark}" stroke-width="0.5" fill="none"/>
    <!-- NECK -->
    <rect x="-3" y="-24" width="6" height="6" fill="#a87850" stroke="${skinDark}" stroke-width="0.4"/>
    <!-- HEAD (looking up at the grip) -->
    <path d="M-8 -34 Q-9 -28 -7 -24 L-3 -21 L3 -21 L7 -24 Q9 -28 8 -34 Z"
          fill="${skin}" stroke="${skinDark}" stroke-width="0.7"/>
    <!-- HAIR -->
    <path d="M-7 -26 Q-6 -23 0 -23 Q6 -23 7 -26 L7 -28 L-7 -28 Z" fill="${o.hair}"/>
    ${cap}
    ${o.accessory === 'glasses' ? `
      <ellipse cx="-3" cy="-28" rx="2.6" ry="2.2" fill="rgba(255,255,255,0.3)" stroke="${skinDark}" stroke-width="0.5"/>
      <ellipse cx="3"  cy="-28" rx="2.6" ry="2.2" fill="rgba(255,255,255,0.3)" stroke="${skinDark}" stroke-width="0.5"/>
      <line x1="-0.5" y1="-28" x2="0.5" y2="-28" stroke="${skinDark}" stroke-width="0.4"/>` : ''}
  </svg>`;
}

// Original standing pose retained for the climb game.
function buildCharSvgInner(o, shirt, pack, accessory, cap) {
  const skin = '#d8a878', skinDark = '#7a4a2a', neckSkin = '#a87850';
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="-36 -54 72 108">
    <!-- BOOTS -->
